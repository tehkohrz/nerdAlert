...
function getUserGroups (userName) {
  const sqlQuery = `SELECT users.id, users.userName, groups.id, groups.id, groups.user_id FROM users INNER JOIN groups ON users.id = groups.user_id HWERE user.userName = ${userName}`;
  const result =  await pool.query(sqlQuery);
   try {
